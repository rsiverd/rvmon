#!/bin/bash
#
# Merge several spectroscopic data sets into a single file for plotting.
#
# Rob Siverd
# Created:      2018-07-14
...
## Required programs:
declare -a need_exec
need_exec+=( awk cat FuncDef getopt sed tr )
...
Usage: $this_prog [options] dataset1 [... datasetN]
Merge multiple spectroscopy datasets into a single file.
...
spec_files=( `ls "$@"` ); errors=$?
if [ $errors -ne 0 ]; then
   recho "Trouble with input files ...\n"
   exit 1
fi
n_files=${#spec_files[@]}

##--------------------------------------------------------------------------##
## Start with header of first file:
first="${spec_files[0]}"
hdrtxt="$(head -1 $first)" || exit $?
#echo "hdrtxt: '$hdrtxt'"
echo $hdrtxt > $foo

## Add non-header lines, sorted by date:
cat "${spec_files[@]}" | grep -v "$hdrtxt" | sort -t, -k2 >> $foo

##--------------------------------------------------------------------------##
## Report or save result:
if [ -z "$save_file" ]; then
   cat $foo
else
...
######################################################################
# CHANGELOG (81_combine_datasets.sh):
#---------------------------------------------------------------------
#
#  2018-07-14:
#     -- Increased script_version to 0.10.
#     -- First created 81_combine_datasets.sh.
#
